...
const jobBaseData = {
    ...
    "Normal Sailor": { name: "Normal Sailor", maxXp: 1000, income: 10},
...
const jobCategories = {
   ...
   // "Alchemy"                :    ["Apprentice Alchemist", "Alchemist", "Master Alchemist"]
}
...
const tooltips = {
    ...
    //sailors
    "Normal Sailor": "A lil placeholder",
    "Sailor Captain": "A lil placeholder",
    "Pirate": "A swashbuckling adventurer",
    "Pirate Captain": "A fearless leader of the high seas",
    "Warlord of the Sea": "A powerful ruler of the ocean",
    "Emperor of the Sea": "A mighty sovereign of the marine realm",
    "King of Pirates": "A legendary pirate lord",
    ...
    for (requiredRow of requiredRows) {
        ...
        if (categoryType.Misc == undefined) {
            for (let i = 0; i < category.length; i++) {
                const entityName = category[i]; // Get the job name
                const requirements = gameData.requirements[entityName]; // Check requirements
        
                if (i >= category.length - 1) break; // Stop if at end of category
        
                if (requirements && i === 0 && !requirements.isCompleted()) {
                    nextEntity = data[entityName];
                    break; // Use break here to exit the loop
                }
        
                // Now, handle the next item in category
                const nextIndex = i + 1;
                if (nextIndex >= category.length) break; // Exit if next index is out of bounds
                ...
                if (nextEntityRequirements && !nextEntityRequirements.isCompleted()) {
                    ...
                    break; // Exit the loop once we set nextEntity
                }
            }
        }
...
gameData.requirements = {
    ...
    // Pirates Guild
    "Normal Sailor": new TaskRequirement([getTaskElement("Normal Sailor")], [{task: "Fisherman", requirement: 10}]),
    "Sailor Captain": new TaskRequirement([getTaskElement("Sailor Captain")], [{task: "Normal Sailor", requirement: 20}, {task: "Strength", requirement: 15}]),
    "Pirate": new TaskRequirement([getTaskElement("Pirate")], [{task: "Sailor Captain", requirement: 15}, {task: "Battle tactics", requirement: 10}]),
    "Pirate Captain": new TaskRequirement([getTaskElement("Pirate Captain")], [{task: "Pirate", requirement: 20}, {task: "Strength", requirement: 30}]),
    "Warlord of the Sea": new TaskRequirement([getTaskElement("Warlord of the Sea")], [{task: "Pirate Captain", requirement: 25}, {task: "Battle tactics", requirement: 50}, {task: "Mana control", requirement: 10}]),
    "Emperor of the Sea": new TaskRequirement([getTaskElement("Emperor of the Sea")], [{task: "Warlord of the Sea", requirement: 30}, {task: "Battle tactics", requirement: 100}, {task: "Strength", requirement: 50}]),
    "King of Pirates": new TaskRequirement([getTaskElement("King of Pirates")], [{task: "Emperor of the Sea", requirement: 40}, {task: "Battle tactics", requirement: 200}, {task: "Strength", requirement: 100}]),
